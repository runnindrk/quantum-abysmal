...
Error DensityOfStates2dCpuStandard::Save() const
{
    LOG_INFO << "Saving DoS ...";
    StorageEngine::SaveDoS();
    return SUCCESS;
}
...
void DensityOfStates2dCpuStandard::InitializeKpmVectors()
{
    ...
    // This migtht be a waste of memory, could I assign directly to a?
    std::vector<double> rngVector =
        rngEngine.GetRandomVector(lattice.hamiltonianSize + numberOfGhosts);
    
    memcpy(a, rngVector.data(), rngVector.size());
    memset(b, 0, rngVector.size());
